...
tran_data = pd.read_csv(download('kaggle_house_train'))
test_data = pd.read_csv(download('kaggle_house_test'))
print(tran_data.shape)
print(test_data.shape)

all_features = pd.concat((tran_data.iloc[:, 1:-1], test_data.iloc[:, 1:]))
...
train_labels = torch.tensor(
    tran_data.SalePrice.values.reshape(-1, 1), dtype=torch.float32)
...
class TestMLPKaggleHousePrice(unittest.TestCase):
    def test_train(self):
